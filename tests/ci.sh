...
# Prepare for running the test suite, whose tests use user namespaces
# both rootless and as root.  In the latter case, a container with a
# limited ID mapping cannot access directories owned by unmapped users,
# such as the runner's home directory the source tree is under.
setup_userns() {
