...
def check_numaif_capabilities(feature: str):
    result = subprocess.run(['tests/tests_mempolicy_helper'], capture_output=True, text=True)
    return True if feature in result.stdout.splitlines() else False

def check_numa_hw():
    return subprocess.run(['numactl', '--show'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode
...
def test_mempolicy_numa_balancing_flag():
    ...
    if not check_numaif_capabilities("MPOL_F_NUMA_BALANCING"):
        return 77
...
def test_mempolicy_static_relative_nodes_flags():
    ...
    if not check_numaif_capabilities("MPOL_F_RELATIVE_NODES") or not check_numaif_capabilities("MPOL_F_STATIC_NODES"):
        return 77
...
def test_mempolicy_no_nodes():
    ...
    conf['process']['args'] = ['/init', 'cat', '/proc/self/numa_maps']
    add_all_namespaces(conf)
    conf['linux']['memoryPolicy'] = { "mode": "MPOL_PREFERRED" }
...
def test_mempolicy_bind_mode_balancing_relative():
    ...
    if not check_numaif_capabilities("MPOL_F_NUMA_BALANCING") or not check_numaif_capabilities("MPOL_F_RELATIVE_NODES"):
        return 77
...
def test_mempolicy_preferred_mode_static():
    ...
    if not check_numaif_capabilities("MPOL_F_STATIC_NODES"):
        return 77
...
def test_mempolicy_preferred_many_mode_all_nodes():
    ...
    conf['process']['args'] = ['/init', 'cat', '/proc/self/numa_maps']
    add_all_namespaces(conf)
    conf['linux']['memoryPolicy'] = { "mode": "MPOL_PREFERRED_MANY", "nodes": "all" }
...
def test_mempolicy_weighted_interleave_mode():
    ...
    if not check_numaif_capabilities("MPOL_WEIGHTED_INTERLEAVE"):
        return 77
...
all_tests = {
    ...
    "mempolicy-preferred-many-mode-all-nodes": test_mempolicy_preferred_many_mode_all_nodes,
