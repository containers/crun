 ...
 * ctx_idx.
 *
 * The `args` field is deliberately not handled: libcrun_context_set_args only
 * takes the argv of the calling process, which libcrun overwrites in place to
 * change the process title.  There is no way to hand it Lua strings.
 * [-0, +0] */
static void
...
luacrun_ctx_status_container (lua_State *S)
{
  ...
  ret = libcrun_status_get_state (status, &state, &running, &crun_err);
  if (ret < 0)
    {
      libcrun_container_status_free (status);
      luacrun_SoftErrIf (S, crun_err != NULL, &crun_err, lua_pushnil (S), 1);
      lua_pushnil (S);
      lua_pushstring (S, "failed to read state");
      return 2;
    }
  ...
  const struct luacrun_string_pair values[] = {
    { "status", libcrun_container_state_to_string (state) },
    { "bundle", libcrun_status_get_bundle (status) },
    { "rootfs", libcrun_status_get_rootfs (status) },
    { "created", libcrun_status_get_created (status) },
    { "systemd-scope", libcrun_status_get_scope (status) }, /* maybe NULL */
    { "owner", libcrun_status_get_owner (status) },         /* maybe NULL */
    { NULL, NULL },
  };
...

  {
    ...
    ret = libcrun_get_state_dir (&dir, libcrun_context_get_state_root (ctx), id, &crun_err);
    if (ret < 0)
      {
    ...
    annotations_len = libcrun_container_get_annotations_len (container);
    if (annotations_len > 0)
      {
        /* Check stack again, we need three available frames here. */
        luaL_checkstack (S, 3, NULL);
        lua_createtable (S, 0, annotations_len);
        int annotations_idx = lua_gettop (S);
        for (size_t i = 0; i < annotations_len; i++)
          {
            const char *key = NULL;
            const char *val = NULL;

            if (libcrun_container_get_annotation_at (container, i, &key, &val) < 0)
              continue;
            lua_pushstring (S, val);
            lua_setfield (S, annotations_idx, key);
          }
...
#define luacrun_CtxStringAccessor(name)                          \
  ...
  {                                                              \
    ...
    lua_getiuservalue (S, 1, LUACRUN_CTX_CACHE_UV);              \
    /* Keep the previous value on the stack, it is returned.  */ \
    lua_getfield (S, -1, #name);                                 \
    if (val != NULL)                                             \
      lua_pushstring (S, val);                                   \
    else                                                         \
      lua_pushnil (S);                                           \
    lua_setfield (S, -3, #name);                                 \
    return 1;                                                    \
  }
